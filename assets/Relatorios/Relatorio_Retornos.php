<?php

#---------------------------------------------------------------------------------------------------------------------------------------
# Formulário de seleção para o relatório de Retornos.
# Envia para o arquivo Processar_Retornos.php o ID do cliente, data inicio do relatório e data fim.
#---------------------------------------------------------------------------------------------------------------------------------------
# Autor: Marcelo Bueno
# Data: 23/11/2020
# Última revisão: 26/12/2020 
#---------------------------------------------------------------------------------------------------------------------------------------

require __DIR__."/../Conexao.php";
require __DIR__."/../Verifica_login.php";

$buscarClientes = "SELECT * FROM `clientes`";
$buscarClientesExec = mysqli_query($conn, $buscarClientes);

?>

<main class="corpo">
        <div class="container">
            <div class="text-center">
                <br>
                <h3>Fechamento - Retornos</h3>
            </div>
            <div class="row mt-5">
                <div class="col col-0 col-md-2 col-lg-2 col-xl-2">

                </div>
                <div class="col col-12 col-md-8 col-lg-8 col-xl-8">
                    <div class="card">
                        <div class="ml-3 mr-3 mt-3 mb-3">
                            <form action="./assets/Relatorios/Processar_Retornos.php" method="post" target="_blank">
                                <label class="font-weight-normal" for="cliente"><b class="text-danger">Selecione o Cliente: </b></label>
                                <select class="form-control" name="cliente" id="">
                                    <?php
                                        while($result = mysqli_fetch_assoc($buscarClientesExec))
                                        { ?>
                                            <option value="<?= $result['id_cliente']; ?>"><?= $result['nome_cliente']; ?></option>
                                        <?php }
                                    ?>
                                </select>
                                <label class="font-weight-normal mt-3" for="data_ini"><b class="text-danger">Selecione a Data Início: </b></label>
                                <input class="form-control" type="date" name="data_ini">
                                <label class="font-weight-normal mt-3" for="data_fim"><b class="text-danger">Selecione a Data Fim: </b></label>
                                <input class="form-control" type="date" name="data_fim">
                                <button class="btn btn-lg btn-success mt-3 float-right" type="submit">Gerar Relatório</button>
                            </form>
                        </div>
                    </div>
                </div>
                <div class="col col-0 col-md-2 col-lg-2 col-xl-2">

                </div>
            </div>
        </div>
</main>